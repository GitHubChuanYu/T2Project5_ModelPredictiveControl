 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N - 1; t++) {
      ...
      // Make sure during sharp turn, speed is reduced for stable driving
      fg[0] += 1000*CppAD::pow((vars[t + v_start] * vars[t + delta_start]), 2);
    }

    // Minimize the value gap between sequential actuations.
    for (int t = 0; t < N - 2; t++) {
      fg[0] += 55 * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Return the first control outputs including steering angle and acceleration/brake
  // and also the predicted x and y positions for plotting MPC predicted trajectory
  // in simulator
